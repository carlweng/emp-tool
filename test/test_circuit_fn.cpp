...
// circuit ONCE (through a RecordContext) into a context-free Circuit, then run
// the SAME Circuit on ClearContext. Exercises both body forms, a nullary
...
// Decode a value evaluated on ClearContext (Wire == uint8_t, 0/1) via its shape.
...
int main() {
    ...
    // run the SAME compiled circuit on ClearContext, two input sets.
    auto run_add = [&](uint32_t A, uint32_t B) {
        auto x = UInt<ClearContext, 32>::constant(cx, A);
        auto y = UInt<ClearContext, 32>::constant(cx, B);
    ...
    auto c2 = cf::compile<UIntShape<32>, UIntShape<32>>(add);
    ...
    auto mul_plus1 = [](auto& ctx, auto a, auto b) {
        ...
        return a * b + UInt<C, 32>::constant(ctx, 1);
    };
    auto cmp = cf::compile<UIntShape<32>, UIntShape<32>>(mul_plus1);
    {
    ...
    auto ck = cf::compile<UIntShape<32>>(add7);
    {
        uint32_t A = 1000u;
        auto x = UInt<ClearContext, 32>::constant(cx, A);
    ...
    // 5) fp32 IR-replay body — compile once, run on ClearContext (clear outputs).
    auto fadd = [](auto a, auto b) { return a + b; };
    auto cfp = cf::compile<FloatShape<32>, FloatShape<32>>(fadd);
    {
        auto x = Float<ClearContext, 32>::constant(cx, 1.5f);
        auto y = Float<ClearContext, 32>::constant(cx, 2.25f);
        chk("fp32 add (compiled replay)", clear_of(cf::run(cx, cfp, x, y)) == 3.75f);
    }

    // 6) live run (implicit form) agrees with compiled run.
    {
        auto x = UInt<ClearContext, 32>::constant(cx, 5u);
        auto y = UInt<ClearContext, 32>::constant(cx, 9u);
