...
//   geq(const Self&)    -> legacy::Bit_T<Wire>
//   equal(const Self&)  -> legacy::Bit_T<Wire>
//   select(const legacy::Bit_T<Wire>&, const Self&) -> Self
...
class Sortable { public:
	...
	legacy::Bit_T<Wire> operator>=(const Self& rhs) const {
...
class IfThenBuilder {
	legacy::Bit_T<Wire> cond_;
	T then_val_;
public:
	IfThenBuilder(const legacy::Bit_T<Wire>& c, const T& t) : cond_(c), then_val_(t) {}
...
class IfBuilder {
	legacy::Bit_T<Wire> cond_;
public:
	explicit IfBuilder(const legacy::Bit_T<Wire>& c) : cond_(c) {}
	template<typename T>
...
template<typename Wire>
inline IfBuilder<Wire> If(const legacy::Bit_T<Wire>& cond) {
	return IfBuilder<Wire>(cond);
}

template<typename T>
inline void swap(const legacy::Bit_T<typename T::wire_type>& swap, T& o1, T& o2) {
...
void cmp_swap(T* key, D* data, int i, int j, legacy::Bit_T<typename T::wire_type> acc) {
	legacy::Bit_T<typename T::wire_type> to_swap = ((key[i] > key[j]) == acc);
...
void bitonic_merge(T* key, D* data, int lo, int n,
                   legacy::Bit_T<typename T::wire_type> acc) {
	if (n > 1) {
...
template<typename T, typename D = legacy::Bit_T<typename T::wire_type>>
void sort(T* key, int size, D* data = nullptr,
          legacy::Bit_T<typename T::wire_type> acc = legacy::Bit_T<typename T::wire_type>(true, PUBLIC)) {
